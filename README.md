...
- `DriveApiClient` も同じ厳格な `Json` デコードのままだったため、Drive API の実際の応答
  （`kind` フィールドなど）でバックアップ・一覧取得・フォルダ作成が毎回失敗する問題
  （同様に `ignoreUnknownKeys = true` へ変更し、エラー応答時は Drive からの実際のエラー内容を
  表示するようにしました）
- アプリ用フォルダの作成リクエストで `mimeType` が送信されておらず（`kotlinx.serialization` は
  デフォルト値と同じプロパティを既定でエンコードしないため）、Drive が「フォルダ」ではなく
  「ただのファイル」を作成してしまい、以降のアップロードが `parentNotAFolder` エラーで失敗する問題
  （`encodeDefaults = true` を設定して解消）
- 「最終同期」がエポックミリ秒の生数値のまま表示されていた問題（日時形式に整形するよう修正）
- Drive からの復元後、写真の実データがローカルに存在しないままで、サムネイル側に
...
- Compose Multiplatform を 1.9.3 に上げた後、Web 版でページ全体がブラウザのネイティブスクロールで
  はみ出すようになり、戻る矢印やボタンをクリックしても反応しない／別の場所を押した扱いになる問題。
  `index.html` の `html, body` に `overflow: hidden` が設定されておらず、canvas がわずかでも
  ビューポートより大きく測定されるとブラウザ側でページスクロールが発生してしまい、それが
  Skiko（Compose Web のレンダリング基盤）のクリック座標計算とズレる、という Compose
  Multiplatform for Web 側の既知の癖です。公式ドキュメントが提示する標準の `styles.css` にも
  `overflow: hidden` が含まれており、これを追加して解消しました
- Android で写真フィールドに画像を追加すると、一部の写真が意図せず横向き（または上下逆）で
  表示される問題。多くのカメラ・ギャラリーアプリは写真をそのままの向きのピクセル列ではなく、
  「実データは横向きのまま、EXIF の Orientation タグで表示時の回転を指示する」形式で保存します。
  `BitmapFactory.decodeByteArray` はこのタグを見ずにピクセルをそのままデコードするため、
...
- Android で Google アカウントへのログインがアプリを再起動するたびに必要になる問題。
  `AndroidGoogleAuthClient` はアプリ起動時に一切サインイン状態を確認しておらず、常に
  「未サインイン」から始まっていました（Web 版が `localStorage` から復元するのとは非対称でした）。
  Google Play Services の `AuthorizationClient` は一度許可した `drive.file` スコープを
  端末側で覚えているため、アプリ起動時に一度だけ「画面を一切出さずに」再認可を試みるよう
  追加し、すでに許可済みであれば自動的にサインイン状態を復元するようにしました
  （未許可の場合は何も起きず、これまで通り「Google でログイン」ボタンから手動でサインインします）
- 一覧画面（ライブラリ一覧・レコード一覧）で写真の読み込みが遅く、スクロールも重い問題。
  原因は3つ重なっていました。(1) `PhotoThumbnail` の画像デコード処理が `LaunchedEffect` の
  既定ディスパッチャ（メインスレッド）でそのまま実行されており、特に直前に追加した EXIF 回転
  処理（ビットマップ全体を作り直す）が加わってメインスレッドを長く塞いでいた、(2) カメラ写真は
  数千 px 四方あるのが普通なのに、48dp 程度の小さなサムネイル表示のためだけに毎回フル解像度で
  デコードしていた、(3) LazyColumn は画面外に大きくスクロールした行を破棄するため、スクロールで
  再び表示されるたびにデコードをやり直していた。それぞれ、デコードを `Dispatchers.Default` に
  逃がす、表示サイズに応じて `BitmapFactory.Options.inSampleSize`（Android）／Skia の
  `Surface` 経由の縮小描画（Web）で必要な解像度までしかデコードしない、`AppContainer` に
  セッション内で使い回すデコード結果のキャッシュ（`ImageBitmapCache`）を持たせる、という形で
  対応しました
- 一覧の写真サムネイルが、ページ再読み込みだけでなく同じセッション内で画面を出入りしても
  一部だけずっと読み込めないまま（プレースホルダーのまま）になる問題。原因は
  `DriveApiClient.downloadBytes` が Drive のエラー応答（レート制限の 429 など、一覧の写真を
  一斉にダウンロードすると起きやすい）のステータスを確認せず、エラー本文（JSON）をそのまま
  「写真のバイト列」として扱ってしまっていたこと。壊れたバイト列がそのままローカル
  キャッシュ（Android のファイル／Web の IndexedDB）に保存され、画像として絶対にデコードできない
  ため、以後はキャッシュヒットするだけで再ダウンロードが起きず、恒久的に表示されなくなって
  いました。`downloadBytes` に他の Drive API 呼び出しと同様のステータスチェックを追加し、
  一覧表示時に写真を同時ダウンロードしすぎないよう Semaphore で同時実行数を絞り、一時的な
  失敗はリトライするようにしました。また `PhotoThumbnail` 側にも、キャッシュ済みバイト列の
  デコードに失敗した場合は一度だけ Drive から強制的に再ダウンロードする自己修復ロジックを
  追加し、この修正より前に壊れたバイト列を掴んでしまった既存のキャッシュも自然に直るように
  しています
- Web 版で、Drive から復元した写真の読み込みが遅い（特にページを再読み込みした直後）問題。
  ...
  確認して検証済みです
- Android で写真を追加して「今すぐバックアップ」を実行しても、その写真を Web 側で（別デバイス
  として）復元すると一覧のサムネイルがまったく表示されない問題。開発者ツールの Network タブで
  確認したところ、そもそも Drive への写真ダウンロードのリクエスト自体が発生していませんでした。
  原因は `SyncService.backupNow()` の処理順序で、`backup.json`（各添付ファイルがどの Drive
  ファイル ID に対応するかを含む）を先にアップロードしてから、その後で未同期の写真バイト列を
  アップロードして `driveFileId` を記録していたことです。つまり、その回のバックアップで新しく
  同期された写真は、写真自体は確かに Drive に上がるものの、`backup.json` にはまだ
  `driveFileId: null` の状態がそのまま書き込まれてしまい、これを読む側（Web など）には
  「どのファイルをダウンロードすればいいか」という情報が永久に欠けたままになっていました。
  写真バイト列のアップロードを `backup.json` の組み立て・アップロードより先に行うよう順序を
  入れ替えて解消しました。この修正は、実際の `DriveApiClient`／`SyncService` を差し替えなしで、
  `ktor-client-mock` で Drive API 応答だけを模擬したスタンドアロンのテストハーネスで検証して
  います。修正前のコードでは実際に `backup.json` の中身が `"driveFileId":null` になることを
  再現でき、修正後は正しく `"driveFileId":"（アップロード先のID）"` になることを確認しました。
